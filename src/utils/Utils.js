...
import { a, ERA, GEOSPARQL, WGS84, RDFS, OWL, RDF, XSD } from './NameSpaces.js';
...
async function getAllOperationalPoints(store) {
    ...
    if (ops) {
        return await Promise.all(Object.keys(ops).map(async op => {
            ...
            const uopid = opp[op][ERA.uopid].value;
            ...
            const flag = emoji_flags[opp[op][ERA.uopid].value.slice(0, 2)]

            return {
                value: op,
                uopid: uopid,
                label_txt: `${Array.isArray(label) ? label.map(l => l.value).join(' / ')
                    : label.value}`,
                label: `${flag} ${Array.isArray(label) ? label.map(l => l.value).join(' / ')
...
function getOPIdFromCoords(lngLat, graphStore, ne) {
    // TODO: Fix this and adjust to a proper way to retrieve the related OP URI
    // Query for OP ID

    const incomingMicroNE = queryGraphStore({
        store: graphStore,
        o: ne
    });

    const outgoingMicroNE = queryGraphStore({
        store: graphStore,
        s: ne
    });
    ...
    if(!locId) {
        console.log(ne);
        console.log(incomingMicroNE);
        console.log(outgoingMicroNE);
    }

    // Hack to deal with poor data quality where 2 or more OPs have the exact same location
    // by comparing the OP URI with the NetElement URI
    if (Object.keys(locId).length > 0) {
        for (const op of Object.keys(locId)) {
            const opid = op.split('/')[op.split('/').length - 1];
            if (ne.indexOf(opid) >= 0) return op;
        }
    } else {
...
function getTrackInfo(t, store) {
    ...
    if (track && track[t]) {
        ...
        // Expand contact line system (if any)
        if (track[t][ERA.contactLineSystem]) {
            let cls = null;
            if (Array.isArray(track[t][ERA.contactLineSystem])) {
                cls = track[t][ERA.contactLineSystem].map(clsId => {
                    const c = queryGraphStore({ s: clsId.value, store });
                    if (c) {
                        c[clsId.value]['@id'] = clsId.value;
                        return c[clsId.value];
                    } else {
                        return { '@id': clsId.value };
                    }
                });
            } else {
                const clsId = track[t][ERA.contactLineSystem].value;
                const clsq = queryGraphStore({ s: clsId, store });
                if (clsq) {
                    clsq[clsId]['@id'] = clsId;
                    cls = clsq[clsId];
                } else {
            ...
            if (cls) {
                track[t][ERA.contactLineSystem] = cls;
            }
        }
        ...
        if (track[t][ERA.trainDetectionSystem]) {
            let tds = null;
            if (Array.isArray(track[t][ERA.trainDetectionSystem])) {
                tds = track[t][ERA.trainDetectionSystem].map(tdsId => {
                    const t = queryGraphStore({ s: tdsId.value, store });
                    if (t) {
                        t[tdsId.value]['@id'] = tdsId.value;
                        return t[tdsId.value];
                    } else {
                        return { '@id': tdsId.value };
                    }
                });
            } else {
                const tdsId = track[t][ERA.trainDetectionSystem].value;
                const tdsq = queryGraphStore({ s: tdsId, store });
                if (tdsq) {
                    tdsq[tdsId]['@id'] = tdsId;
                    tds = tdsq[tdsId];
                } else {
